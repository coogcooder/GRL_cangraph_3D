...
class GraphDesignEnv:
    def __init__(
        ...
        *,
        ...
        self.span_limits_x = cfg_meta.get("x_min_max", (8.0, 30.0))
    ...
    def step(self, action, action_str: str = None, *, mode: str = None):
        ...
        if mode == "local":
            if not getattr(self, "_node_index_map", None):
                self.observe_tensors()
            if not self._node_index_map:
                obs = self.observe_tensors()
                return obs, 0.0, False
            node_idx = int(getattr(action, "local_node", 0).item()) % len(self._node_index_map)
            node_id = list(self._node_index_map.keys())[node_idx]
            self._apply_local_delta_safe(node_id, delta)
        elif mode == "global":
...
class ReinforcementDesignAgent:
    ...
    def _global_remove_column(self, girder_id: str):
        ...
        if removed_any:
            self._merge_girder_segments(root_gid, xi, yi)
            self._graph_state = None
            self._node_index_map = {}
            if self._design_graph is not None:
                self._design_graph.apply_global_action(self.problem, self.cfg_meta, {})
    ...
    def _merge_girder_segments(self, girder_id: str, xi: float, yi: float):
