...
BUILDING_WIDTH_RANGE = (60.0, 120.0)
BUILDING_LENGTH_RANGE = (40.0, 80.0)
BUILDING_STORY_RANGE = (3, 6)
BUILDING_STORY_HEIGHT_RANGE = (3.0, 3.0)
BAY_SPACING_RANGE = (6.5, 8.5)
SEC_SPACING_RANGE = (2.0, 2.5)
SPAN_LIMITS_X = (8.0, 14.0)


def validate_cfg_meta(cfg_meta: dict) -> None:
    span_limits = cfg_meta.get("span_limits_x")
    if span_limits is None or span_limits[0] >= span_limits[1]:
        raise ValueError("cfg_meta['span_limits_x'] must define an increasing range")
    story_height = cfg_meta.get("story_h")
    if story_height is None or story_height <= 0:
        raise ValueError("cfg_meta['story_h'] must be positive")
    bay_spacing_range = cfg_meta.get("bay_spacing_range")
    if bay_spacing_range is None or bay_spacing_range[0] <= 0 or bay_spacing_range[1] <= 0:
        raise ValueError("cfg_meta['bay_spacing_range'] must contain positive values")
    sec_spacing_range = cfg_meta.get("sec_spacing_range")
    if sec_spacing_range is None or sec_spacing_range[0] <= 0 or sec_spacing_range[1] <= 0:
        raise ValueError("cfg_meta['sec_spacing_range'] must contain positive values")
    ...
    *,
    ...
    for ep in range(total_episodes):
        ...
        cfg_meta = {
            ...
            "w_dead": cfg["w_dead"],
            "w_live": cfg["w_live"],
            "span_limits_x": SPAN_LIMITS_X,
            "x_min_max": SPAN_LIMITS_X,
            "y_min_max": BAY_SPACING_RANGE,
            "width_x": build.width_x,
            "length_y": build.length_y,
            "bay_spacing": bay_spacing,
            "sec_spacing": sec_spacing,
            "width_range": BUILDING_WIDTH_RANGE,
            "length_range": BUILDING_LENGTH_RANGE,
            "story_range": BUILDING_STORY_RANGE,
            "story_h": build.story_h,
            "story_height_range": BUILDING_STORY_HEIGHT_RANGE,
            "bay_spacing_range": BAY_SPACING_RANGE,
            "sec_spacing_range": SEC_SPACING_RANGE,
        }

        validate_cfg_meta(cfg_meta)
